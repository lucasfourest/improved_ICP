...
def get_paths(args):
    ...
    ref,data= files[0], files[1]

    return dir+'/'+ref, dir+'/'+data
...
def rmse(data_aligned, matched_ref):
    sq_distances = np.sum(np.power(data_aligned - matched_ref , 2), axis=0)
...
def show_ICP(data, ref, R_list, T_list, neighbors_list):
    ...
    def draw_event():
        ...
        if dim == 2:
            ...
            plt.axis('equal')
        if dim == 3:
            ax.plot(ref[0], ref[1], ref[2], '.', alpha=0.05)
            ax.plot(data_aligned[0], data_aligned[1], data_aligned[2], '.', alpha=0.05)
            #plt.axis('equal')
        if show_neighbors and iteration < max_iter:
